			...
			"BareMetal" {
				...
				switch ($DriverInstallMode) {
                    ...
                    "Hybrid" {
                        ...
                        elseif (@(Get-ChildItem -Path $RootDriverDirectoryPath | Where-Object {$_.Name -match "^(sp\d+|manifest.json|manifest.xml)$"}).Count -eq @(Get-ChildItem -Path $RootDriverDirectoryPath).Count) {
